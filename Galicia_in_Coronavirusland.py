...
df2['Galicia'] = df2['Galicia'].truediv(2.701743).round()
...
for index, row in df4.iterrows():
    country = row[0]
    permil = row[1]
    result[country] = result[country].truediv(permil).round()
...
#      PLOT THE RESULTS       #
###############################
# This is just a minimalistic sample
# Check the MatPlotLib documentation for fancier options
# We will provide an auxiliary script specifically from 
# plotting the results from the newly generated CSV file
# There is no need to download and format the data
# each time we want to plot it
...
result.plot(kind='line',x='dateRep',y='Galicia',ax=ax)
result.plot(kind='line',x='dateRep',y='Spain', color='red',ax=ax)
result.plot(kind='line',x='dateRep',y='Germany',ax=ax)
result.plot(kind='line',x='dateRep',y='Italy',ax=ax)
result.plot(kind='line',x='dateRep',y='United_Kingdom',ax=ax)
result.plot(kind='line',x='dateRep',y='Netherlands',ax=ax)
result.plot(kind='line',x='dateRep',y='France',ax=ax)

# We save the plot in SVG (editable vectorial graph)
# and PNG formats

plt.savefig(cwd + '/Galicia_in_Coronavirusland.svg')
plt.savefig(cwd + '/Galicia_in_Coronavirusland.png')
